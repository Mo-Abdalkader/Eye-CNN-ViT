...
from models.neural_networks import create_dual_output_model
from training.trainer import (
    train_one_epoch, validate, setup_training_components, setup_dataloaders
)
from training.early_stopping import EarlyStopping
from training.evaluation import plot_training_history, plot_confusion_matrices, save_model

def run_training_pipeline(model_type='ResNet50', augment=False, fine_tune=True):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")
    
    train_dataset, val_dataset, class_weights = prepare_datasets(augment=augment)
    ...
    history = {'train_loss': [], 'val_loss': [], 'train_acc_ret': [], 'val_acc_ret': [], 'train_acc_mac': [], 'val_acc_mac': []}

    early_stopper = EarlyStopping(patience=5, mode='min', verbose=True, path=f"{model_type.lower()}_{str(augment)}_early_stopper_best_weights.pth")

    for epoch in range(Config.EPOCHS):
        print(f"\nEpoch {epoch+1}/{Config.EPOCHS}")
        
        train_loss, train_acc_ret, train_acc_mac = train_one_epoch(model, train_loader, optimizer, criterion_ret, criterion_mac, device)
        ...
        if val_loss < best_val_loss:
            ...
            print()
            save_model(model, f"{model_type.lower()}_{str(augment)}_best_model.pth")
            # save_weights(model, f"{model_type.lower()}_{str(augment)}_best_weights.pth")
    ...
    if fine_tune:
        ...
        if model_type == 'ResNet50':
            ...
            layers_to_unfreeze = getattr(Config, 'UNFREEZE_LAYERS_ViT', 3)
        ...
        early_stopper = EarlyStopping(patience=5, mode='min', verbose=True, path=f"{model_type.lower()}_{str(augment)}_early_stopper_best_weights_finetuned.pth")
            
        for epoch in range(Config.EPOCHS_TUNE):
            print(f"\nFine-tune Epoch {epoch+1}/{Config.EPOCHS_TUNE}")
            ...
            if val_loss < best_val_loss:
                best_val_loss = val_loss
                save_model(model, f"{model_type.lower()}_{str(augment)}_best_model_finetuned.pth")
                # save_weights(model, f"{model_type.lower()}_{str(augment)}_best_weights_finetuned.pth")
